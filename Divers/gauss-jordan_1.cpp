...
public:
    ...
    vector<vector<long long>> Matrix_int;
    vector<Frac> Dir_Vec;
    vector<int> Leading_zeros;
    vector<bool> Ordered;
    bool Echlonned;
    bool Reduced;
    Gauss_Matrix(vector<vector<long long>> init_gauss_matrix_int) :
        Matrix_int(init_gauss_matrix_int) {
            size_t row = init_gauss_matrix_int.size();
            size_t col = init_gauss_matrix_int.at(0).size();
            vector<vector<Frac>> tmp(row, vector<Frac>(col));
            for(size_t i = 0; i < row; ++i) {
                for(size_t j = 0; j < col; ++j) {
                    tmp[i][j] = Frac(init_gauss_matrix_int[i][j]);
                }
            }
            Gauss_Matrix(tmp);
        }
